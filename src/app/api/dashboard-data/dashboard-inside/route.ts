...
export async function GET(req: Request) {
    ...
    try {
      ...
      if (searchCategory === "Expenses") {
        ...
        // Calculate savings
        const monthsInRange = currentDate.getMonth() - startDate.getMonth() + 1 + (12 * (currentDate.getFullYear() - startDate.getFullYear()));
        const totalExpenses = expenses.reduce((acc, expense) => acc + expense.amount, 0);
        const monthlySalary = user.monthlySalary;
        const totalSavings = (monthlySalary * monthsInRange) - totalExpenses;
        result = totalSavings;
